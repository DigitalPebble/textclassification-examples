package com.digitalpebble.classification.xml;
...
public class XMLCorpusClassifier extends DefaultHandler {
	...
	public XMLCorpusClassifier(String input, String resourceDir)
			throws Exception {
	...
	public static void main(String[] args) {
		...
		String model = args[1];
		try {
			XMLCorpusClassifier reader = new XMLCorpusClassifier(input, model);
	...
	public void endElement(String nsURI, String strippedName, String tagName)
		...
		if (tagName.equalsIgnoreCase("document")) {
			...
			try {
				...
				System.out.println("best label > "
						+ classifier.getBestLabel(scores));
			...
			Field f = new Field(currentFieldName, currentString.toString()
					.split(regexpSplitter));
			fields.add(f);
		} else if (tagName.equalsIgnoreCase("label")) {
